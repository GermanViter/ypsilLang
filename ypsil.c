...
void lenv_load_file(lenv* e, const char* filepath, mpc_parser_t* parser) {
    mpc_result_t r;
    if (mpc_parse_contents(filepath, parser, &r)) {
        lval* expr = lval_read(r.output);
        mpc_ast_delete(r.output);
...
void load_stdlib(lenv* env, mpc_parser_t* parser) {
    char cwd[PATH_MAX];
    ...
    snprintf(filepath, sizeof(filepath), "%s/prelude.ypsl", cwd);

    lenv_load_file(env, filepath, parser);
...
int main(int argc, char** argv) {
  ...
  if (argc == 1) {
    ...
    puts("Ypsil Version 1.0");
    puts("Press Ctrl+c to Exit\n");
  
    while (1) {
    
      char* input = readline("Ypsil> ");
      add_history(input);
